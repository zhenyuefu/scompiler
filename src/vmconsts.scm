...
(define (generate-vm-constants)
  (let ((hfile (open-output-file "constants.h" #:exists 'replace))
        (cfile (open-output-file "constants.c" #:exists 'replace)))
...
(define (generate-header file)
  (display "/* fichier généré automatiquement : *ne pas éditer* */" file)
  (newline file)
